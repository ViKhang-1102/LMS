...
try {
    ...
    if ($isAdmin) {
        ...
        $query = "SELECT s.id AS submission_id, s.assignment_id, s.user_id, s.content, s.file_path, s.status, s.grade, 
                         s.submitted_at, c.title AS course_title, a.title AS assignment_title,
                         g.feedback, g.graded_at
                  FROM submissions s
                  JOIN assignments a ON s.assignment_id = a.id
                  JOIN courses c ON a.course_id = c.id
                ...
                <tbody>
                    <?php foreach ($submissions as $submission): ?>
                        <tr>
                            ...
                            <td class="course-column"><?php echo htmlspecialchars($submission['assignment_title']); ?></td>
                            <?php if ($isAdmin): ?>
                            ...
                            <td class="grade-column">
                                ...
                                <?php if ($isAdmin && $submission['status'] === 'pending'): ?>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="/controllers/grading_action.php" method="POST">
                                        ...
                                        <input type="hidden" name="course_id" value="<?php echo $submission['course_id'] ?? 0; ?>">
                                        <div class="mb-3">
                                            <label for="grade<?php echo $submission['submission_id']; ?>" class="form-label">Grade (0-100)</label>
                                            <input type="number" class="form-control" id="grade<?php echo $submission['submission_id']; ?>" name="grade" min="0" max="100" required>
                                        </div>
                                        <div class="mb-3">
